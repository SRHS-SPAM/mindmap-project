...
import { MessageSquare, Zap, Target, BookOpen, Link, Settings } from 'lucide-react';

/* global __initial_auth_token */

// ==========================================================
// [인증 토큰 및 프로젝트 ID 설정]
// 1. 환경 변수 __initial_auth_token을 우선 사용합니다.
// 2. 환경 변수가 없을 경우 (로컬 테스트 등) 하드코딩된 mock 토큰을 사용합니다.
// ==========================================================
const INITIAL_AUTH_TOKEN = typeof __initial_auth_token !== 'undefined'
    ? __initial_auth_token
    : "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0ZXN0QGdlbWluaS5jb20iLCJleHAiOjE3OTM1MzY0MDB9.j0zH0qW-V9J8hG0YtL7c9-WlR1p2Y2c7Q6N2Lw7v8w4"; // 만료 기한 연장된 Mock Token
    
const API_BASE_URL = 'http://localhost:8000/api/v1';
const PROJECT_ID = 1; // 테스트용 프로젝트 ID
const PROJECTS_ENDPOINT = `${API_BASE_URL}/projects`;

// HTTP 요청 헬퍼: 설정된 INITIAL_AUTH_TOKEN 사용
const fetchApi = async (url, method = 'GET', body = null) => {
    const headers = {
        'Content-Type': 'application/json',
        // 수정된 부분: INITIAL_AUTH_TOKEN을 사용
...
        // 401 에러를 위한 상세 메시지 추가
        if (response.status === 401) {
             throw new Error("인증 실패 (401 Unauthorized): 토큰이 유효하지 않거나 만료되었습니다. 백엔드의 security.py 설정을 확인해 주세요.");
        }
        throw new Error(errorDetail.detail || `API 요청 실패: ${response.status} ${response.statusText}`); 
    }
    // No Content 응답 처리
